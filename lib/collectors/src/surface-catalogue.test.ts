...
describe("collector surface catalogue", () => {
  ...
  it("has exactly five live collectors: source, dependency, tls, certificate and kms", () => {
    // `dependency` became live when B2's ingest path landed, `tls` when B3's
    // did (`POST /projects/:id/tls`), `certificate` when B4's did
    // (`POST /projects/:id/certificates`), and `kms` when B5's did
    // (`POST /projects/:id/kms`). A collector with nowhere to write
