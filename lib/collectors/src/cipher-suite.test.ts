...
describe("parseCipherSuite — positive cases", () => {
  it("reads key exchange, authentication and record protection off an IANA TLS 1.2 name", () => {
    ...
    expect(parsed?.components).toEqual([
      { role: "key-exchange", algorithm: "ECDH/DH", token: "ECDHE", keySize: undefined },
  ...
  it("reads the same three components off OpenSSL's spelling of the same suite", () => {
    expect(algorithmFor("ECDHE-RSA-AES128-GCM-SHA256", "key-exchange")).toBe("ECDH/DH");
  ...
  it("maps DSS to DSA and every DH spelling to the single ECDH/DH entry", () => {
    ...
    for (const name of [
      ...
      expect(algorithmFor(name, "key-exchange"), name).toBe("ECDH/DH");
    }
  });
...
describe("parseCipherSuite — the false positives it must refuse", () => {
  ...
  it("does NOT invent an authentication algorithm for an anonymous suite", () => {
    ...
    expect(algorithmFor("TLS_DH_anon_WITH_AES_128_CBC_SHA", "key-exchange")).toBe("ECDH/DH");
  });
  ...
  it("does NOT invent an authentication algorithm for a PSK suite", () => {
    ...
    expect(algorithmFor("TLS_ECDHE_PSK_WITH_AES_128_GCM_SHA256", "key-exchange")).toBe("ECDH/DH");
