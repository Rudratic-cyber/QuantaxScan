...
6. **Coverage Meter (D3)**: Two journeys in `ui-journey.spec.ts` asserting the meter states the size of the blind spot in words ("N of 13 have never been examined" — the spec derives both numbers from `COLLECTOR_SURFACES` rather than hard-coding them, so adding a surface cannot silently make the sentence wrong), keeps examined-and-found-nothing distinct from never-examined, reports failed attempts without counting them as coverage, and renders a *refusal* rather than a reassuring zero when the coverage call fails.
7. **Posture Timeline (D7)**: `tests/ui/timeline-journey.spec.ts`, four journeys, all of which are about wording and about the absence of a comforting reading:
   - **No collection at all** — "No history to draw", the server's "an empty inventory, not a clean one" sentence printed verbatim, and hard assertions that no breach count, no scrubber and no chart appear. A zeroed chart here would claim a clean estate nobody has examined.
   - **One collection instant** — the "No trend" banner, and `getByTestId("observed-series")` asserted at **count 0**. That is the assertion stopping a future change from joining one point to itself, or to the projection, and calling it a trend. The measurement itself is still asserted visible, because hiding a real reading is its own dishonesty.
   - **Three instants** — three observed series (one per scenario, never blended), keyboard scrubbing with `Home`/`End` from the oldest measurement into the projection, the `Observed`→`Projected` badge flip, the assumption restated on the projected frame, both IR 8547 markers with their key strengths, draft label and citation link, and the estate roll-up including assets belonging to no project.
   - **A failed read** — "Timeline could not be read", with `/No history to draw/` asserted absent: "we could not ask" and "there is nothing there" are different answers.

   The populated journey stubs `/api/projects/*/coverage` with a *well-formed empty* meter payload rather than `{}`. The Overview tab mounts first and `CoverageMeter` dereferences `confidence` unconditionally, so a hollow stub blanks the whole page before the tab bar renders — a failure that looks like a D7 bug and is not.
...
| `04-certificates` (B4 / **G-22**) | Extended: the same openssl-generated certificates are read back through `GET /api/inventory/timeline`, so the estate-wide count is proven against a real stack rather than only per project |
