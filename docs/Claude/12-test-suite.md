...
6. **Coverage Meter (D3)**: Two journeys in `ui-journey.spec.ts` asserting the meter states the size of the blind spot in words ("8 of 10 have never been examined"), keeps examined-and-found-nothing distinct from never-examined, reports failed attempts without counting them as coverage, and renders a *refusal* rather than a reassuring zero when the coverage call fails.
