/**
 * Aceternity UI — MacBook Scroll (faithful recreation)
 * Imports from framer-motion (installed), not motion/react.
 */
...
export interface MacbookScrollProps {
  /** JSX rendered inside the screen — use this for interactive content */
  screenContent?: ReactNode;
  /** Fallback image URL if screenContent not provided */
  src?: string;
...
export const Lid = ({
  scaleX, scaleY, rotate, translate, screenContent, src,
}: {
  ...
  return (
    <div className="relative [perspective:800px]">
      {/* Closed lid (visible before scroll) */}
      <div
        style={{
      ...
      {/* Opening lid (animates on scroll) */}
      <motion.div
        style={{
          scaleX,
          scaleY,
      ...
      >
        <div className="absolute inset-0 rounded-lg bg-[#0d0d14] overflow-hidden">
          {screenContent ? (
            <div className="w-full h-full overflow-hidden">{screenContent}</div>
          ) : src ? (
            <img
              src={src}
              alt="screen"
