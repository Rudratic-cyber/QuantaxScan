...
function QuantaXscanMark({ size = 32 }: { size?: number; glow?: boolean; animate?: boolean }) {
  const s = size;
  const cx = s / 2;
  const cy = s / 2;
  const r = s * 0.44;
...
export function QuantaXscanLogo({
  ...
  if (variant === "icon") return <QuantaXscanMark size={s.icon} />;
