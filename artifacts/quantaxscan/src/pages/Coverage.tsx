...
import { Boxes, Check, Clock, ArrowRight, ShieldCheck } from "lucide-react";
...
// The ten collector surfaces are no longer listed here. They come from
// `@workspace/collectors/surface-catalogue` via `@/lib/collector-surfaces`,
// which is the same list the per-project coverage meter (D3) counts against.
// Two copies of this list is how the page came to claim eight surfaces while
// the roadmap said ten.
const COLLECTORS = COLLECTOR_SURFACE_LIST;
      ...
      <section className="border-b border-[#eceef2] bg-[#f7f8fa]">
        <div className="container mx-auto flex flex-col items-start gap-3 px-4 py-6 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-[#475569]">
            <span className="font-semibold text-[#0a0e1a]">{now} of {TOTAL_SURFACE_COUNT} surfaces available now.</span>{" "}
          ...
          {COLLECTORS.map((c, i) => (
            <Reveal key={c.name} delay={(i % 2) * 0.06}>
              <div className="flex h-full gap-4 rounded-2xl border border-[#e5e7eb] bg-white p-6">
                <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl ${c.status === "live" ? "bg-[#eef0fe] text-[#4f46e5]" : "bg-[#f1f3f7] text-[#9aa3b2]"}`}>
                  {c.icon}
                </div>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-base font-semibold text-[#0a0e1a]">{c.name}</h3>
                    <StatusPill status={c.status === "live" ? "now" : "planned"} />
                  </div>
